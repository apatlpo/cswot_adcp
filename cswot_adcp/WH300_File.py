...
class WH300_File:
    def __init__(self, fname=None, year=1970):
        ...
        istart = np.where((tout[:-4] == 0x7F) * (tout[1:-3] == 0x7F) * (tout[4:] == 0))[
            0
        ]
    ...
    def ParseBH(data):
        ...
        data = data[0, :]
        return dict(
            NbBytes=data[2] + 256 * data[3],
            NbData=data[5],
            DTOffsets=np.array(
                [data[6 + u * 2] + 256 * data[6 + u * 2 + 1] for u in range(data[5])]
            ),
        )
    ...
    def ParseVariable(data, year):
        ...
        Magic = (
            int("3040", 16)
            if ((int("3040", 16) <= Magic) * (Magic <= int("30FC", 16)))
            else Magic
        )
    ...
    def ParseFL(data):
        ...
        return dict(
            FWVer=data[2] + data[3] / 100,
            SysConf=data[4] + data[5] * 256,
            Frequency={0: 75, 1: 150, 2: 300, 3: 600, 4: 1200, 5: 2400, 6: 38}[
                data[4] & 0x7
            ],
            Convex=((data[4] & 8) == 8),
            SnsConf=(data[4] & 48) >> 4,
    ...
    def ParseVL(data, year):
        ...
        y[y == 0] = year
        return dict(
            ENum=data[:, 2] + 256 * data[:, 3] + 65536 * data[:, 11],
            time=[
                datetime(*d, tzinfo=pytz.utc).timestamp()
                for d in zip(
                    y,
                    data[:, 5],
                    data[:, 6],
            ...
            Temperature=(((data[:, 26] + data[:, 27] * 256 + 32768) % 65536) - 32768)
            / 100,
            MPTMin=data[:, 28],
    ...
    def ParseBT(data):
        VD = (
            ((data[:, 24:32:2] + 256 * data[:, 25:32:2] + 32768) % 65536) - 32768
        ) / 1000
        VD[np.abs(VD) > 20] = np.NaN
        return dict(
            NumPings=data[:, 2] + 256 * data[:, 3],
            CorrMin=data[:, 6],
            AmpMin=data[:, 7],
            Mode=data[:, 9],
            EVelMax=(data[:, 10] + 256 * data[:, 11]) / 1000,
            Range=(data[:, 16:24:2] + 256 * data[:, 17:24:2] + 65536 * data[:, 77:81])
            / 100,
            Vel=VD,
            Corr=data[:, 32:36],
            Amp=data[:, 37:40],
    ...
    def ParseNav(data):
        return dict(
            FirstUTC=np.array(
                [
                    datetime(*d, tzinfo=pytz.utc).timestamp()
                    for d in zip(data[:, 4] + 256 * data[:, 5], data[:, 3], data[:, 2])
                ]
            )
            + (
                data[:, 6]
            ...
            * 180
            / 2**31,
            SMG=((data[:, 40] + 256 * data[:, 41] + 2**15) % 2**16 - 2**15)
            * 0.001,
            CMG=((data[:, 42] + 256 * data[:, 43] + 2**15) % 2**16 - 2**15)
            * 180
            / 2**15,
            ...
            EPCTime=np.array(
                [
                    datetime(*d, tzinfo=pytz.utc).timestamp()
                    for d in zip(
                        data[:, 54] + 256 * data[:, 55], data[:, 57], data[:, 56]
                    )
                ]
            )
            + (
            ...
            / 100,
            Pitch=(((data[:, 62] + 256 * data[:, 63] + 2**15) % 2**16) - 2**15)
            * 180
            / 2**15,
            Roll=(((data[:, 64] + 256 * data[:, 65] + 2**15) % 2**16) - 2**15)
            * 180
            / 2**15,
            Heading=(((data[:, 66] + 256 * data[:, 67] + 2**15) % 2**16) - 2**15)
            * 180
            / 2**15,
        )
    ...
    def ParseFA(data):
        return dict(
            EE=np.hstack(
                [
                    np.hstack((data[:, u : u + 1] // 16, data[:, u : u + 1] % 16))
                    for u in range(2, 10)
                ]
            ),
            ...
            EH=np.hstack(
                [
                    (
                        (((data[:, 11] + data[:, 12] * 256 + 32768) % 65536) - 32768)
                        / 100
                    )[:, None],
                    data[:, 13:14],
                ]
            ...
            EP=np.hstack(
                [
                    (((data[:, 18] + data[:, 19] * 256 + 32768) % 65536) - 32768)[
                        :, None
                    ]
                    / 100,
                    (((data[:, 20] + data[:, 21] * 256 + 32768) % 65536) - 32768)[
                        :, None
                    ]
                    / 100,
                    data[:, 22][:, None],
